<x-layout>
    <x-slot:content>
        <div class="container" style="font-family: Arial, sans-serif; margin-top: 20px; max-width: 800px; padding: 20px; background-color: #f9f9f9; border-radius: 8px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
            <h1 style="text-align: center; color: #333; font-size: 2em; margin-bottom: 20px;">Review Cafe</h1>

            {{-- Create Review Form --}}
            <form method="POST" action="{{ route('musician.review.store') }}">
                @csrf
                <div class="mb-5">
                    <label for="cafe_id" class="block mb-2 text-sm font-medium text-gray-900" style="font-size: 1.1em; color: #555;">ID Pemilik Cafe</label>
                    <input type="number" name="cafe_id" id="cafe_id" class="block w-full p-4 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 text-base focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
                </div>
