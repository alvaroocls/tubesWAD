<x-layout>
    <x-slot:content>
        <div class="container mt-5" style="max-width: 600px; background-color: #f9f9f9; padding: 20px; border-radius: 10px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); margin: 0 auto;">
            ...
            <form action="{{ route('musician.profile.update', $musician->id) }}" method="POST" enctype="multipart/form-data" style="font-size: 14px;">
                @csrf
                @method('PUT')
...
{{-- <x-layout>
    <x-slot:content>
        <div class="container mt-5">
            <h1 class="mb-4">Edit Musician Profile</h1>
            ...
            <form action="{{ route('musician.profile.update', $musician->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')

                <div class="mb-3">
                    <label for="name" class="form-label">Name</label>
                    <input type="text" class="form-control" id="name" name="name" value="{{ old('name', $musician->name) }}" required>
                </div>

                <div class="mb-3">
                    <label for="genre" class="form-label">Genre</label>
                    <input type="text" class="form-control" id="genre" name="genre" value="{{ old('genre', $musician->genre) }}" required>
                </div>

                <div class="mb-3">
                    <label for="location" class="form-label">Location</label>
                    <input type="text" class="form-control" id="location" name="location" value="{{ old('location', $musician->location) }}" required>
                </div>

                <div class="mb-3">
                    <label for="bio" class="form-label">Bio</label>
                    <textarea class="form-control" id="bio" name="bio" rows="4">{{ old('bio', $musician->bio) }}</textarea>
                </div>
        ...
        <style>
            /* Membuat teks di input dan textarea menjadi hitam */
            input, textarea {
                color: black;
            }
        </style>
