<x-layout>
    <x-slot:content>
        ...
        <div class="mx-32">
            <h1 class="text-4xl font-bold mb-12">Edit job listing</h1>
            
        <form action="{{ route('cafeOwner.postingjob.update', $job->id) }}" method="post" class="max-w-sm" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-5">
                <label for="title" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Job title</label>
                <input type="text" id="title" name="title" value="{{ $job->title }}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light" required />
            </div>
            ...
            <div class="mb-5">
                ...
                <input type="date" id="date" name="date" value="{{ $job->date }}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
            </div>

            <div class="mb-5">
                <label for="time" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Time</label>
                <input type="time" id="time" name="time" value="{{ $job->time }}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
            </div>
            ...
            <fieldset class="flex flex-wrap gap-4">
                ...
                @php
                    $preferences = explode(',', $job->preferences);
                @endphp

                <div class="flex items-center mb-4">
                    <input id="checkbox-acoustic" type="checkbox" name="preferences[]" value="Acoustic Performance" {{ in_array('Acoustic Performance', $preferences) ? 'checked' : '' }} class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                    <label for="checkbox-acoustic" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">Acoustic Performance</label>
                </div>
            
                <div class="flex items-center mb-4">
                    <input id="checkbox-band" type="checkbox" name="preferences[]" value="Band Performance" {{ in_array('Band Performance', $preferences) ? 'checked' : '' }} class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
            ...
            <div class="mb-5">
                ...
                <input class="mt-4 block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400" aria-describedby="file_input_help" id="image" name="image" type="file">
                <p class="mt-1 text-sm text-gray-500" id="file_input_help">SVG, PNG, JPG, Max file size 2MB</p>
                @if($job->image)
                    <p class="mt-1 text-sm">Current image: <a href="{{ asset('img/' . $job->image) }}" target="_blank">View Image</a></p>
                @endif
            </div>

            <button type="submit" class="my-8 w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Update</button>
