<x-layout>
    <x-slot:content>
        <div class="container" style="font-family: Arial, sans-serif; margin: 20px auto; max-width: 800px; padding: 20px; background-color: #f9f9f9; border-radius: 8px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
            <h1 style="text-align: center; color: #333; font-size: 2em; margin-bottom: 20px;">Review Musisi yang Manggung</h1>

            {{-- Daftar review yang diberikan oleh cafe owner --}}
                ...
                @else
                    @foreach ($reviews as $review)
                        <div style="margin-bottom: 20px;">
                            ...
                            <p><em>Untuk Musisi: {{ $review->musician->name ?? 'Musisi' }}</em></p>
                            
                            {{-- Tombol Update dan Delete --}}
                            <div style="margin-top: 10px;">
                                <a href="{{ route('cafeOwner.review.edit', $review) }}" style="padding: 5px 10px; background-color: #ffa500; color: white; text-decoration: none; border-radius: 5px; font-size: 0.9em; margin-right: 10px;">
                                    Edit Deskripsi
                                </a>
                                
                                <form action="{{ route('cafeOwner.review.destroy', $review) }}" method="POST" style="display: inline;">
                                    @csrf
                                    @method('DELETE')
            ...
            <div style="text-align: center; margin-top: 20px;">
                <a href="{{ route('cafeOwner.review.create') }}" style="display: inline-block; padding: 10px 20px; background-color: #4CAF50; color: white; text-decoration: none; border-radius: 5px; font-size: 1em;">
                    Tambah Review
                </a>
            </div>
        </div>
